...
def wait_for_pod_name_ready(pod_name, pod_namespace):
    ...
    try:
        ...
        result = subprocess.run(
            f"oc wait --for=condition=ready pod/{pod_name} -n {pod_namespace} --timeout=300s", shell=True
        )

        if result.returncode == 0:
            pass_message(f"The pod({pod_name}) in '{pod_namespace}' namespace are running and ready.")
            return 0
        else:
...
def check_oc_status():
    ...
    try:
        ...
        for groupName in groupNames:
            result = subprocess.run("oc get clusterrolebindings -o json", shell=True, capture_output=True, text=True)
            clusterAdmin = sum(
                1
