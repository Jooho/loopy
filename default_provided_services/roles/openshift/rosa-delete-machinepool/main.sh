...
info "rosa delete machinepool --cluster=$CLUSTER_NAME $MACHINE_POOL_NAME -y"
...
info "Verify if the machinepool is removeded"
while true; do
  ...
  if [ -z "$MACHINE_POOL_EXISTS" ]; then
    ...
    info "Machine pool($MACHINE_POOL_NAME) is still being deleted..."
  fi
...
if [[ $result == 0 ]]; then
  success "Machine pool($MACHINE_POOL_NAME) has been deleted."
else
  error "Cluster failed to delete the machinepool($MACHINE_POOL_NAME)"
fi
