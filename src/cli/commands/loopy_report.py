...
def summary(ctx, type, config_data, unit_list):
    ...
    total_seconds = round(float(execution_time % 60), constants.EXECUTION_TIME_ROUND)
    ...
    execution_time_sec_list = [round(float((end - start) % 60), constants.EXECUTION_TIME_ROUND) for start, end in zip(start_time, end_time)]
    ...
    with open(f"{report_file}", "r") as file:
        ...
        for line in data:
            ...
            if prev_role_index == role_index and prev_role_name == role_name:
                ...
                total_sec_time += round(execution_time_sec_list[int(target_index)+temp_index] ,constants.EXECUTION_TIME_ROUND)
                temp_index+=1
                same_role_count+=1
                ...
                if total_sec_time !=0 and target_index != None:
                    ...
                    temp_execution_time_sec_list.append(round(total_sec_time,constants.EXECUTION_TIME_ROUND))
        ...
        if "unit" in step:
            ...
            for role in unit_steps:
                if "shell-execute" == role['role']['name']:
                    command_count=role['role']['input_env']['COMMANDS'].count("%%")+1
    ...
    with open(f"{report_file}", "r") as file:
        ...
        for line in data:
            ...
            if type == "playbook":
                ...
                description = getDescription(ctx, role_name, accumulate_time_index_per_unit_role, to_check_duplicate_role_name, config_data, unit_list)                    
                
            if result_line in py_step_time_position_index_list:
                accumulate_time_index_per_unit_role = py_step_time_position_index_list.index(result_line)          
                unit_description = py_step_unit_role_description[accumulate_time_index_per_unit_role]
                table.add_row(
                [
                    f"{Fore.YELLOW}Step({accumulate_time_index_per_unit_role})",
                    f"{unit_description}",
                    f"Check following result",
                    f"{py_step_min_time_list[accumulate_time_index_per_unit_role]}min {round(py_step_sec_time_list[accumulate_time_index_per_unit_role],constants.EXECUTION_TIME_ROUND)}sec{Fore.RESET}",
                    "",
                ]
            )
