...
def deploy_isvc(protocol, namespace, verbose, directory, deployment_mode):
    ...
    response_json = kserve.get(
        name=name, timeout_seconds=600, namespace=namespace, watch=False
    )
    endpoint = (
    ...
    if endpoint is not None:
        ...
        raise ValueError(
            "Failed to deploy the isvc - not able to find the endpoint, check the response: "
            + response_json
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="Serverless",
        type=str,
        help="Serverless or RawDeployment",
    )
    parser.add_argument(
        "-n", "--namespace", type=str, help="Models isvc target namespace"
    )
    parser.add_argument(
        "-v",
        "--verbose",
