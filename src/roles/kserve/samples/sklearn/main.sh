...
errorHappened=$(wait_pod_containers_ready "serving.kserve.io/inferenceservice=${ISVC_NAME}" ${TEST_NAMESPACE})
  ...
  if [[ $temp_result == "0" ]]
  then
    success "[SUCCESS] ${MODEL_NAME} runtime is running "
    result=0
  else
    error "[FAIL] ${MODEL_NAME} runtime is NOT running"
    errorHappened=0
  fi
  ...
  then
    ...
    if [[ $model_result == "200" ]] || [[ $model_result == 0 ]]
    then
      success "[SUCCESS] ${MODEL_NAME} runtime return correct data"
      result=0
    else
      error "[FAIL] ${MODEL_NAME} runtime DID NOT return correct data"
      result=1
      errorHappened=0
    fi
  fi
