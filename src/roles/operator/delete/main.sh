...
if [[ ${OPERATOR_NAMESPACE} == 'openshift-operators' || $(is_positive "${CSV_DELETE}") == 0 ]]; then
...
if [[ $(is_positive "${CATALOGSOURCE_DELETE}") == 0 ]]; then
...
result=$?
if [[ $result == 1 ]]; then
  success "Operator(${OPERATOR_NAME}) is removed"
  result=0
else
  error "Operator(${OPERATOR_NAME}) still exist"
fi
