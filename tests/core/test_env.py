...
@pytest.fixture(params=[
    ("/tmp", "custom.yaml", "/tmp", "/tmp/custom.yaml"),  # When both LOOPY_ROOT_PATH and LOOPY_CONFIG_NAME are set
    (None, "custom.yaml", get_default_loopy_root_path(), get_default_loopy_root_path()+"/custom.yaml"),  # When LOOPY_ROOT_PATH is not set
    ("/tmp", None, "/tmp", "/tmp/config.yaml"),  # When LOOPY_CONFIG_PATH is not set
    (None, None, get_default_loopy_root_path(), get_default_loopy_root_path()+"/config.yaml"),  # When both are not set
])
def env_manager_with_params(monkeypatch, request):
    loopy_root_path, loopy_config_name, expected_loopy_root_path, expected_config_path = request.param
...
def test_env_manager_initialization(env_manager_with_params):
    ...
    env_manager, expected_loopy_root_path, expected_config_path = env_manager_with_params
...
def test_loopy_env_vars():
    ...
    env_manager = EnvManager()  # A new instance to reflect the updated environment variables.
