...
def test_validate_config_yaml_schema(mock_env_and_config, loopy_root_path):
    ...
    errors = initializer.validate_config_yaml_schema(
        f"{loopy_root_path}/tests/test-data/schema/success-role-config.yaml", "role"
    )
    assert len(errors) == 0  # No errors should be present

    errors = initializer.validate_config_yaml_schema(
        f"{loopy_root_path}/tests/test-data/schema/fail-role-config.yaml", "role"
    )
    assert len(errors) == 1
    ...
    # Unit
    errors = initializer.validate_config_yaml_schema(
        f"{loopy_root_path}/tests/test-data/schema/success-unit-config.yaml", "unit"
    )
    assert len(errors) == 0  # No errors should be present
    ...
    # Playbook
    errors = initializer.validate_config_yaml_schema(
        f"{loopy_root_path}/tests/test-data/schema/success-playbook-config.yaml",
        "playbook",
    )
    assert len(errors) == 0  # No errors should be present

    errors = initializer.validate_config_yaml_schema(
        f"{loopy_root_path}/tests/test-data/schema/fail-playbook-config.yaml",
        "playbook",
    )
    assert len(errors) == 1
    assert (
