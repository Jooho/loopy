...
def test_default_cert_generation(role_dir, role_env, setup_test_env):
    ...
    test_env = {
        "SAN_DNS_LIST": "minio.minio.svc.cluster.local,minio-minio.test",
        "SAN_IP_LIST": "192.168.1.1",
    }
    ...
    result = subprocess.run(
        [str(role_dir / "main.sh")], env=env, capture_output=True, text=True
    )
    ...
    verify_result = subprocess.run(
        [
            ...
            "verify",
            "-CAfile",
            str(role_dir_path / role_env["ROOT_CA_CERT_NAME"]),
            str(cert_path),
        ],
        capture_output=True,
        text=True,
...
def test_cert_generation_with_existing_ca(role_dir, role_env, setup_test_env, tmp_path):
    ...
    verify_result = subprocess.run(
        ["openssl", "verify", "-CAfile", str(ca_dir / "root.crt"), str(cert_path)],
        capture_output=True,
        text=True,
    )
    assert verify_result.returncode == 0
